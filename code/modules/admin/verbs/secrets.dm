	...
	switch(action)
		...
		if("customportal")
			...
			var/list/settings = list(
				"mainsettings" = list(
					"typepath" = list("desc" = "Path to spawn", "type" = "datum", "path" = "/mob/living", "subtypesonly" = TRUE, "value" = /mob/living/simple_animal/hostile/bear),
			...
			if (prefreturn["button"] == 1)
				...
				var/mutable_appearance/storm = mutable_appearance('icons/obj/energy_ball.dmi', "energy_ball_fast", FLY_LAYER)
