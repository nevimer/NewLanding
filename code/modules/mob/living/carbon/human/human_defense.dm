		...
		for(var/thing in body_part.wounds)
			...
			switch(W.severity)
				if(WOUND_SEVERITY_TRIVIAL)
					msg = "\t [SPAN_DANGER("Your [body_part.name] is suffering [W.a_or_from] [W.get_topic_name(src)].")]"
				if(WOUND_SEVERITY_MODERATE)
					msg = "\t [SPAN_WARNING("Your [body_part.name] is suffering [W.a_or_from] [W.get_topic_name(src)]!")]"
				if(WOUND_SEVERITY_SEVERE)
					msg = "\t [SPAN_WARNING("<b>Your [body_part.name] is suffering [W.a_or_from] [W.get_topic_name(src)]!</b>")]"
