...
/datum/round_event_control/antagonist/canSpawnEvent(popchecks = TRUE)
	. = ..()
	if(!.)
		return
...
/datum/round_event_control/antagonist/solo/canSpawnEvent(popchecks = TRUE)
	. = ..()
	if(!.)
		return
...
/datum/round_event/antagonist
	fakeable = FALSE
	endWhen = 60 //This is so prompted picking events have time to run //TODO: refactor events so they can be the masters of themselves, instead of relying on some weirdly timed vars
