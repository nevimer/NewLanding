	...
	if(href_list["admin_pref"])
		switch(href_list["admin_pref"])
			if("handle")
				...
				if(last_ckey && last_ckey != usr.ckey)
					var/action = tgui_alert(
						usr,
						"[last_ckey] is already handling this review! Do you want to handle nonetheless?",
						null,
						list("Yes", "No")
					)
			...
			if("discard_review")
				var/action = tgui_alert(
					usr,
					"Are you sure you want to discard this review request (Use request changes if you want it changed instead)?",
					null,
					list("Yes", "No")
				)
	...
	if(href_list["pref"])
		switch(href_list["pref"])
			...
			if("edit_admin_note")
				...
				if(msg)
					note_to_admins = strip_html_simple(msg, MAX_AMBITION_FLAVOR_LEN, TRUE)
					log_action("NOTE: [note_to_admins]", FALSE)
			if("set_narrative")
				var/msg = input(usr, "Set your narrative!", "Narrative", narrative) as message|null
				if(msg)
					narrative = strip_html_simple(msg, MAX_AMBITION_FLAVOR_LEN, TRUE)
			...
			if("edit_objective")
				...
				if(msg)
					if(length(objectives) < index)
						return
					objectives[index] = strip_html_simple(msg, MAX_AMBITION_FLAVOR_LEN, TRUE)
					log_action("OBJ - edit: [old_obj] TO-> [objectives[index]]")
			if("add_objective")
				var/msg = input(usr, "Add new objective:", "Objectives", "") as message|null
				if(msg)
					var/new_obj = strip_html_simple(msg, MAX_AMBITION_FLAVOR_LEN, TRUE)
					objectives += new_obj
